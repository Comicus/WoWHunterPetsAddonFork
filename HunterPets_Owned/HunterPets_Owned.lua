...
function HunterPetsOwned_GenerateTooltip(anchor, name, subtext, displayId, npc)
	...
	if (duplicates) then
		...
		OwnedToolTip:SetCell(OwnedToolTip:AddLine(), 1, "Found duplicate:", GameFontSmall, "LEFT", 0)
...
function HunterPetsOwned_Update()
	...
	HunterPetsOwnedStats:SetText(numPets .. " pets (" .. numPetsScanned .. " fully scanned)")
	...
	for i=1, totalSlots do
		...
		if (HunterPetsGlobal.data[selectedRealm][selectedCharacter].OwnedPets[i]) then	
			...
			button.tooltip = "Empty Stable Slot"
...
function HunterPetsOwned_UpdateSlot(button, slotIndex)
	...
	button.tooltip = name;
	button.tooltipSubtext = format("Level %1$d %2$s", level, family);
